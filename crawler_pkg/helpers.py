...
class scraper():
    ...
    def crawl(self, max_urls):
        ...
        self.pbar.update(self.total_urls_visited)
        for link in links:
            # for link in links:
    ...
    def parse_forms(self, soup):
        ...
        # for i, form in tqdm(enumerate(forms, start=1), desc="parsing forms",  leave=True):
        for i, form in enumerate(forms, start=1):
            form_details = self.__get_form_details(form)

            self.all_forms = self.all_forms.new_child(OrderedDict(form_details))
    ...
    def __check_emails(self):
        ...
        for email in self.email_unclear:
            try:
                self.emails.add(validate_email(email, check_deliverability=True, dns_resolver=None).email)
            except:
                continue
    ...
    def parse_phone_numbers(self, soup):
        ...
        numbers = phonenumbers.PhoneNumberMatcher(soup.html.decode(), self.country_code)  # FIXME
    ...
    def parse_datetime(self, text_obj, parser):
        ...
        for i in range(len(preds)):
            ...
            start = start - 10
            if end > len(text_obj): end = len(text_obj)
            if start < 0: start = 0
    ...
    def begin(self, max_urls):
        self.pbar = tqdm(total=max_urls, desc="crawling")
        self.crawl(max_urls)
        self.pbar.close()
        self.__check_emails()
        self.__dump_data()
